...
class ValidationImages:
    ...
    def mapping_imagens_with_template():
        pattern = ValidationImages.get_images_in_database()
        image_test = ValidationImages.get_image_in_Vrep()

        if pattern and image_test:
            cv.matchTemplate(image_test, pattern, cv.TM_CCOEFF_NORMED)
